...
| Homebrew authoring | `features/class-builder/*` | ✅ Phase 8: табовый builder на authoring-контракте; смонтирован для admin и homebrew; обе legacy-визарда удалены; без сырого JSON |
| Admin tools | `pages/admin/ContentQualityPage.tsx`, `ConfirmDeleteClassDialog` | ✅ Phase 9: data-quality инспектор с фильтрами + защищённое удаление класса |
| Character sheet (runtime data) | `FolioPage` + `CompatibilityBanner` + `lib/characterMigration.ts` | ✅ Phase 10: compat-баннер по сигналу миграции, read-only блок при `blocked`, smoke-тесты |
...
- **Phase 8 (cont.)** ✅ authoring-контракт получен → построен новый табовый **ClassBuilder**
  ...
  **все 8 grant-типов** с типовыми контролами (dropdown'ы из reference, free-text для custom/modifierKey,
  ...
  `CharacterClassesPage`), **legacy `AdminClassRichWizard` удалён**. `tsc`/`build`/`eslint`/`vitest` зелёные (29/29).
- **Phase 8 (done)** ✅ builder смонтирован и для **homebrew** scope (`HomebrewClassBuilder` в
  `EditDoctrinePage`, scope `{kind:'homebrew', packageId}`); **legacy `homebrew/RichClassWizard` удалён**.
  Оба пути авторства классов теперь на новой content-модели, без сырого JSON. `tsc`/`build`/`eslint`/`vitest` зелёные.
- **Phase 9** ✅ admin tools & data management. Страница `ContentQualityPage` (`/admin/content-quality`,
  пункт навигации `nav.contentQuality`): таблица reward groups по всем классам с фильтрами
  (класс / уровень / grant type / статус) и data-quality сводкой (классы, группы, неизвестные grant,
  пустые группы, группы с ошибками); валидация по классу через `classDetailToDraft` + `validateClassDraft`.
  Защита деструктива: `ConfirmDeleteClassDialog` — превью каскадных дочерних записей (features/groups/
  options/grants) + явное подтверждение, удаление через authoring `DELETE` (409 при ссылках). Подключено
  в `CharacterClassesPage` вместо прямого `mutate`. `tsc`/`build`/`eslint`/`vitest` зелёные (29/29).
- **Phase 10** ✅ runtime-данные персонажей. В `CharacterResponse` добавлен опциональный сигнал
  миграции (`contentMigrationStatus`/`contentMigrationBlocked`/`contentMigrationMessage`); `lib/characterMigration.ts`
  — `characterMigrationState` (absent/MIGRATED → ok, без ложных баннеров) + safe-аксессоры листа
  (stats/classLevels/skills/spells/hp/wallet/rewards, рендерят оба варианта данных). `CompatibilityBanner`
  смонтирован в `FolioPage`; level-up получает read-only `RiteGate` при `blocked`. Баннер само-ретируется,
  когда статус MIGRATED/absent (временный compat-путь убирать не нужно). 8 smoke-тестов. `tsc`/`build`/`eslint`/`vitest` зелёные (37/37).
- **Phase 11** ✅ финальный переход + cleanup. (A) Удалён alias `RewardGroupRenderer`/`RewardGroupRendererProps`
  (все на `RewardGroupView`). (B) Из `LevelUpWizardPage` удалён весь legacy ceremony-рендер
  (`OathGroup`/`OathCard`/`ChoiceGroup`/`OfferingCard`/`EmptyChoiceNote`/`AutoGrantRow`/`AsiGroup`),
  legacy-стейт (`choiceSelections`/`asi`/`AsiAllocation`) и legacy-валидация; `StepRewards` теперь
  content-only (HP/derived тайлы + content-rites + `RewardGroupPicker`), gate только по `contentValid`;
  превью классов читает content-поля. (C) Из типа `RewardGroup` удалены legacy-поля
  `rewardType`/`isChoice`/`rewards`; `contentAdapters` (`rewardGroupKey`/`rewardGroupLabel`/`normalizeRewardGroup`)
  и `draftPreview` переведены на content-поля (id/groupKind). `tsc`/`build`/`eslint`/`vitest` зелёные (37/37).
  Остаток (Phase 12): legacy-типы (`RewardEntry`/`RewardSelection`/`LevelUpRequest`/`RichClass*`),
  старые admin-api (`createRichCharacterClass`/import-json), legacy class-detail допущения в типах.
- **Phase 12** ✅ удаление legacy non-bestiary модели. Удалены legacy-типы (`LevelUpRequest`,
  `RewardSelection`, `AbilityScoreImprovementRequest`, `StatIncrease`, `RewardEntry`,
  `RichClassRewardType`, `RichClass*Request`, `CreateRichCharacterClassRequest`, `RichCharacterClassResponse`);
  старые api-методы авторства классов (`admin.api`: `createRichCharacterClass`/`updateRichCharacterClass`/
  `importRichCharacterClassJson`; `homebrew.api`: `createRich*`/`updateRich*`/`importRich*`); legacy-хуки
  (`useCreateRichHomebrewClass`/`useUpdateRichHomebrewClass`/`useImportRichHomebrewClassJson`,
  `useDeleteCharacterClass`). `tsc`/`build`/`eslint`/`vitest` зелёные (37/37).
  **Остаточные `@deprecated`-поля class-detail** (`primaryAbilityStatId`/`savingThrowStatNames`/
  `skillChoiceOptions`/`armorWeaponProficiencies`) намеренно оставлены как **опциональный backward-compat
  ВХОД** для `normalizeClassDetail` (мостит legacy→new); UI рендерит только новые поля. Убираются, когда
  бэкенд гарантированно отдаёт new-only payload.

---

## Definition of Done — статус

- [x] Character creation использует новые content IDs (read-flow на финальном detail).
- [x] Level-up использует reward groups / options / grants (commit через `ContentLevelUpRequest`).
- [x] Homebrew authoring пишет новые content-таблицы (табовый builder, без JSONB-формы).
- [x] Admin-инструменты на новой content-модели (ClassBuilder + ContentQualityPage).
- [x] Runtime-путь не вызывает старые non-bestiary level-up/authoring endpoints.
- [x] Старые API-клиенты / адаптеры / код-пути под legacy reward-rendering удалены.
- [x] Нет допущений «плоские reward entries / rewardType-only» (поля удалены из `RewardGroup`).
- [~] «Один primary ability / объединённый proficiency»: UI не использует; `@deprecated` входные
      поля оставлены для нормализатора до new-only гарантии бэкенда.
- [x] Bestiary не тронут.

---

## Post-audit — закрытые расхождения

После сверки с контрактами закрыты 4 пункта (отдельными коммитами):
1. **Серверные 422-issues → бейджи по path.** `ClassBuilderModal` парсит `ValidationIssue[]` из 422 и
   мержит их в бейджи/review (Save-gate — только по клиентским ошибкам).
2. **Reorder** групп и опций в rewards-builder (`MoveButtons` + `moveItem`).
3. **Reference-эндпоинты по контракту §2:** `referenceApi.getAbilities`/`getFeatOptions`/`getModifierKeys`
   (`/reference/abilities|feats|modifier-keys`); `useBuilderRefData` тянет abilities/skills/feats/spells/
   modifier-keys из `/reference/*`. Бэкенд должен отдавать эти пути (контракт это и предполагал).
4. **If-Match + Idempotency-Key:** `getForEdit` отдаёт ETag → update шлёт `If-Match`; create шлёт
   `Idempotency-Key` (генерится на открытие builder’а).
