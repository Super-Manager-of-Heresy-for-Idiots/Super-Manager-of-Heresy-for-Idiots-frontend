...
export function useEnableAdminRace() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['races', 'admin'] });
      toast.success('Race enabled');
    },
    onError: (error: AxiosError<ApiError>) => {
      toast.error(error.response?.data?.message || 'Failed to enable race');
    },
  });
}
...
export function useDisableAdminRace() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['races', 'admin'] });
      toast.success('Race disabled');
    },
    onError: (error: AxiosError<ApiError>) => {
      toast.error(error.response?.data?.message || 'Failed to disable race');
    },
  });
}
...
export function useCreateHomebrewRace() {
  ...
  return useMutation({
    ...
    onSuccess: (_, vars) => {
      ...
      toast.success('Homebrew race created!');
    },
    onError: (error: AxiosError<ApiError>) => {
      toast.error(error.response?.data?.message || 'Failed to create homebrew race');
    },
  });
}
...
export function useUpdateHomebrewRace() {
  ...
  return useMutation({
    ...
    onSuccess: (_, vars) => {
      qc.invalidateQueries({ queryKey: homebrewListKey(vars.packageId) });
      qc.invalidateQueries({ queryKey: ['homebrew-my', vars.packageId] });
      toast.success('Homebrew race updated!');
    },
    onError: (error: AxiosError<ApiError>) => {
      toast.error(error.response?.data?.message || 'Failed to update homebrew race');
    },
  });
}
...
export function useEnableHomebrewRace() {
  ...
  return useMutation({
    ...
    onSuccess: (_, vars) => {
      qc.invalidateQueries({ queryKey: homebrewListKey(vars.packageId) });
      qc.invalidateQueries({ queryKey: ['homebrew-my', vars.packageId] });
      toast.success('Homebrew race enabled');
    },
    onError: (error: AxiosError<ApiError>) => {
      toast.error(error.response?.data?.message || 'Failed to enable homebrew race');
    },
  });
}
...
export function useDisableHomebrewRace() {
  ...
  return useMutation({
    ...
    onSuccess: (_, vars) => {
      qc.invalidateQueries({ queryKey: homebrewListKey(vars.packageId) });
      qc.invalidateQueries({ queryKey: ['homebrew-my', vars.packageId] });
      toast.success('Homebrew race disabled');
    },
    onError: (error: AxiosError<ApiError>) => {
      toast.error(error.response?.data?.message || 'Failed to disable homebrew race');
    },
  });
}
...
export function useDuplicateSystemRace() {
  ...
  return useMutation({
    ...
    onSuccess: (_, vars) => {
      qc.invalidateQueries({ queryKey: homebrewListKey(vars.packageId) });
      qc.invalidateQueries({ queryKey: ['homebrew-my', vars.packageId] });
      toast.success('System race duplicated into homebrew');
    },
    onError: (error: AxiosError<ApiError>) => {
      toast.error(error.response?.data?.message || 'Failed to duplicate race');
    },
  });
}
