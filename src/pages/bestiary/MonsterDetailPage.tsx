  ...
  return (
    <Frame>
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 10, marginBottom: 18 }}>
        <button className="ao-btn ao-btn--ghost ao-btn--sm" onClick={() => navigate(-1)}><ChevronLeft size={14} /> Назад</button>
        <div style={{ flex: 1 }} />
        {canEdit && monster && (
          <button className="ao-btn ao-btn--primary ao-btn--sm" onClick={() => navigate('edit')}><Pencil size={13} /> Редактировать</button>
        )}
      </div>

      {query.isLoading && (
        <div style={{ textAlign: 'center', padding: '60px 0', fontFamily: 'var(--font-serif)', fontStyle: 'italic', color: 'var(--ink-quiet)' }}>Загрузка статблока…</div>
      )}
      {query.isError && (
