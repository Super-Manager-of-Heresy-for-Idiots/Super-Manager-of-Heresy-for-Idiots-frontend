...
import { DICTIONARY_META } from '@/components/bestiary/constants';
...
export default function BestiaryDictionariesPage() {
  ...
  const kindMeta = DICTIONARY_META.find((k) => k.slug === activeKind)!;
  ...
  return (
    <div style={{ minHeight: '100%', background: 'var(--stone)' }}>
      <header style={{ minHeight: 64, borderBottom: '1px solid var(--rule)', display: 'flex', alignItems: 'center', gap: 12, padding: '12px clamp(16px, 3vw, 32px)', background: 'linear-gradient(180deg, var(--panel) 0%, var(--stone) 100%)' }}>
        <Diamond size={9} />
        <div>
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 15, letterSpacing: 'var(--track-eng)', textTransform: 'uppercase', color: 'var(--ink-bright)' }}>Справочники бестиария</div>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--ink-faint)' }}>/api/admin/bestiary/dictionaries</div>
        </div>
      </header>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 0 }} className="bd-split">
        <nav style={{ width: 260, flexShrink: 0, borderRight: '1px solid var(--rule)', background: 'var(--abyss)', padding: '12px 0' }} className="bd-nav">
          {DICTIONARY_META.map((k) => {
            ...
            const count = (data?.[k.slug] ?? []).length;
            return (
              <button key={k.slug} onClick={() => { setActiveKind(k.slug); setQuery(''); }} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 12, padding: '11px 18px', background: on ? 'linear-gradient(90deg, rgba(176,141,78,0.1), transparent)' : 'transparent', border: 'none', borderLeft: `2px solid ${on ? 'var(--gold)' : 'transparent'}`, cursor: 'pointer', textAlign: 'left' }}>
                <Diamond size={6} color={on ? 'var(--gold)' : 'var(--bronze)'} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontFamily: 'var(--font-serif)', fontSize: 15, fontWeight: 600, color: on ? 'var(--ink-bright)' : 'var(--ink)' }}>{k.label}</div>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--ink-faint)' }}>{k.sub}</div>
                </div>
        ...
        <main style={{ flex: 1, minWidth: 0, padding: 'clamp(16px, 3vw, 24px)' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            <div>
              <div style={{ fontFamily: 'var(--font-serif)', fontSize: 24, fontWeight: 600, color: 'var(--ink-bright)' }}>{kindMeta.label}</div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--ink-faint)' }}>{activeKind} · {rows.length} записей</div>
            </div>
            <div style={{ flex: 1 }} />
            <div style={{ position: 'relative', width: 220 }}>
              ...
              <input className="ao-input" placeholder="Поиск…" value={query} onChange={(e) => setQuery(e.target.value)} style={{ padding: '8px 12px 8px 34px' }} />
            </div>
            <button className="ao-btn ao-btn--primary" onClick={openCreate}><Plus size={13} /> Добавить</button>
          </div>
            ...
            <table className="ao-table" style={{ minWidth: 520 }}>
              <thead>
                <tr>
                  ...
                  <th>Название (рус)</th>
                  <th>Название (англ)</th>
                  {isSources && <th style={{ width: 100 }}>Книга</th>}
              ...
              <tbody>
                {rows.map((e) => (
                  <tr key={e.id}>
                    <td><span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--gold-pale)' }}>{e.code}</span>{e.isUnique && <span className="ao-chip" style={{ marginLeft: 8 }}>уник.</span>}</td>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                        <button className="ao-iconbtn" title="Редактировать" onClick={() => openEdit(e)}><Pencil size={14} /></button>
                        <button className="ao-iconbtn" title="Удалить" onClick={() => setConfirmDel(e)} style={{ borderColor: 'rgba(179,70,26,0.4)', color: '#d8896a' }}><Trash2 size={14} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
                {!isLoading && rows.length === 0 && <tr><td colSpan={isSources ? 5 : 4} style={{ textAlign: 'center', padding: '40px 0', fontFamily: 'var(--font-serif)', fontStyle: 'italic', color: 'var(--ink-quiet)' }}>Записей нет.</td></tr>}
                {isLoading && <tr><td colSpan={isSources ? 5 : 4} style={{ textAlign: 'center', padding: '40px 0', fontFamily: 'var(--font-serif)', fontStyle: 'italic', color: 'var(--ink-quiet)' }}>Загрузка…</td></tr>}
              </tbody>
            </table>
          </div>
        </main>
      ...
      {draft && (
        <Modal title={draft.id ? 'Редактирование записи' : 'Новая запись'} sub={`${kindMeta.label} · DictionaryEntryRequest`} onClose={() => setDraft(null)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <Field label="Код (code)" required hint="≤60, уникален в пределах области">
            ...
            <Field label="Название (рус)" required>
              <input className="ao-input" value={draft.nameRusloc} onChange={(e) => setDraft({ ...draft, nameRusloc: e.target.value })} placeholder="Дракон" />
            </Field>
            ...
            {isSources && (
              <Field label="Код книги (bookCode)" hint="только для источников, ≤20">
            ...
            <button onClick={() => setDraft({ ...draft, isUnique: !draft.isUnique })} style={{ display: 'inline-flex', alignItems: 'center', gap: 9, background: 'transparent', border: 'none', cursor: 'pointer', padding: 0, alignSelf: 'flex-start' }}>
              ...
              <span style={{ fontSize: 14, color: 'var(--ink)' }}>Уникальная запись (isUnique)</span>
            </button>
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 22 }}>
            <button className="ao-btn ao-btn--ghost" onClick={() => setDraft(null)}>Отмена</button>
            <button className="ao-btn ao-btn--primary" disabled={createMut.isPending || updateMut.isPending} onClick={saveDraft}><Save size={13} /> Сохранить</button>
          </div>
        </Modal>
      )}

      {confirmDel && (
        <Modal title="Удалить запись?" sub="Действие необратимо" danger onClose={() => setConfirmDel(null)}>
          <p style={{ fontFamily: 'var(--font-serif)', fontSize: 16, color: 'var(--ink)', margin: 0, lineHeight: 1.6 }}>
            Запись <span style={{ color: 'var(--ink-bright)' }}>«{confirmDel.nameRusloc}»</span> <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--ink-faint)' }}>({confirmDel.code})</span> будет удалена из справочника «{kindMeta.label}».
          </p>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 22 }}>
            <button className="ao-btn ao-btn--ghost" onClick={() => setConfirmDel(null)}>Отмена</button>
            <button className="ao-btn ao-btn--danger" disabled={deleteMut.isPending} onClick={() => deleteMut.mutate({ kind: activeKind, id: confirmDel.id }, { onSuccess: () => setConfirmDel(null) })}><Trash2 size={13} /> Удалить</button>
          </div>
        </Modal>
      )}
