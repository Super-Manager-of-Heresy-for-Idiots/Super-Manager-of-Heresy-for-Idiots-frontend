import type { DictModule } from '../translations';

/**
 * "Бой" (Battle) feature — campaign battle tab: assembly wizard, initiative
 * join, turn tracker, and the player action panel. UI chrome only.
 */
export const battle: DictModule = {
  ru: {
    /* ── Tab + status ───────────────────────────────────── */
    'battle.tabLabel': 'Бой',
    'battle.status.ASSEMBLING': 'Сбор',
    'battle.status.ACTIVE': 'Идёт бой',
    'battle.status.COMPLETED': 'Завершён',

    /* ── Empty / start ──────────────────────────────────── */
    'battle.empty.player.title': 'Боя пока нет',
    'battle.empty.player.body': 'Когда Мастер начнёт сражение, оно появится здесь.',
    'battle.empty.gm.title': 'Сражений нет',
    'battle.empty.gm.body': 'Соберите отряд противников и начните новый бой.',
    'battle.start': 'Начать бой',
    'battle.create.title': 'Новый бой',
    'battle.create.namePlaceholder': 'Название сражения',
    'battle.create.confirm': 'Создать',
    'battle.create.cancel': 'Отмена',
    'battle.create.defaultName': 'Новое сражение',

    /* ── Assembly (GM) ──────────────────────────────────── */
    'battle.assembly.title': 'Сбор боя',
    'battle.assembly.groupTitle': 'Отряд противников',
    'battle.assembly.monstersTitle': 'Бестиарий',
    'battle.assembly.empty': 'Добавьте монстров из списка справа.',
    'battle.assembly.add': 'Добавить',
    'battle.assembly.remove': 'Убрать',
    'battle.assembly.confirmStart': 'Начать бой',
    'battle.assembly.cancelBattle': 'Отменить сбор',
    'battle.assembly.searchPlaceholder': 'Поиск монстра…',
    'battle.assembly.campaignMonsters': 'Монстры кампании',
    'battle.assembly.publicMonsters': 'Общий бестиарий',
    'battle.assembly.noMonsters': 'Монстры не найдены.',
    'battle.assembly.needMonsters': 'Добавьте хотя бы одного монстра',

    /* ── Stats preview ──────────────────────────────────── */
    'battle.stats.avgDanger': 'Ср. опасность',
    'battle.stats.totalXp': 'Всего опыта',
    'battle.stats.monsters': 'Монстров',
    'battle.stats.xpOverride': 'Опыт за бой',
    'battle.stats.xpAuto': 'Авто',
    'battle.stats.xpEditHint': 'Можно изменить вручную',
    'battle.stats.resetXp': 'Сбросить',
    'battle.stats.cr': 'CR',

    /* ── Tracker ────────────────────────────────────────── */
    'battle.tracker.title': 'Очередь хода',
    'battle.tracker.round': 'Раунд {n}',
    'battle.tracker.current': 'Сейчас ходит',
    'battle.tracker.init': 'Иниц.',
    'battle.tracker.initHidden': 'Инициатива монстра скрыта мастером',
    'battle.tracker.empty': 'Пока никто не присоединился к бою.',
    'battle.tracker.you': 'Вы',
    'battle.tracker.conc': 'Конц.',
    'battle.tracker.concHint': 'Концентрация на заклинании',
    'battle.tracker.monster': 'Монстр',
    'battle.tracker.waiting': 'Ожидание игроков…',
    'battle.tracker.hp': 'ОЗ',

    /* ── Join (player) ──────────────────────────────────── */
    'battle.join.title': 'Вступить в бой',
    'battle.join.pickCharacter': 'Выберите персонажа',
    'battle.join.noCharacters': 'У вас нет персонажей в этой кампании.',
    'battle.join.initiative': 'Бросок d20',
    'battle.join.rollDie': 'Бросить кубик',
    'battle.join.manualHint': 'Введите значение d20 (1–20) или бросьте кубик',
    'battle.join.serverRollHint': 'Оставьте пустым — сервер бросит d20; или введите свой (1–20).',
    'battle.join.computed': 'Итоговая инициатива',
    'battle.join.formula': 'd20 {d20} · бонус {bonus}',
    'battle.join.confirm': 'Вступить',
    'battle.join.addAnother': 'Добавить ещё персонажа',
    'battle.join.allJoined': 'Все ваши персонажи уже в бою.',

    /* ── Player action panel ────────────────────────────── */
    'battle.action.title': 'Ваш ход',
    'battle.action.resources': 'Ресурсы',
    'battle.action.abilities': 'Способности',
    'battle.action.ability.class': 'Класс',
    'battle.action.ability.item': 'Предметы',
    'battle.action.ability.use': 'Использовать способность',
    'battle.action.ability.blocked': 'Чтобы использовать: {reason}',
    'battle.action.ability.manualOnly': 'вручную',
    'battle.action.ability.previewLoading': 'Считаем план умения…',
    'battle.action.ability.unavailable': 'Умение недоступно',
    'battle.action.ability.consumes': 'расход',
    'battle.action.spells': 'Заклинания',
    'battle.action.slots': 'Ячейки заклинаний',
    'battle.action.slot.level': 'Ур. {n}',
    'battle.action.economy': 'Действия',
    'battle.action.economy.action': 'Действие',
    'battle.action.economy.bonus': 'Бонусное',
    'battle.action.economy.reaction': 'Реакция',
    'battle.action.economy.used': 'Потрачена',
    'battle.action.economy.free': 'Доступна',
    'battle.action.attacks': 'Атаки',
    'battle.action.items': 'Предметы',
    'battle.action.item.use': 'Использовать',
    'battle.action.item.used': 'Предмет использован',
    'battle.action.item.failed': 'Не удалось использовать предмет',
    'battle.action.item.hint': 'Расходуемые предметы (зелья, свитки). Сервер проверит, что предмет расходуемый.',
    'battle.action.effects': 'Активные эффекты',
    'battle.action.endTurn': 'Закончить ход',
    'battle.action.noResources': 'Нет ресурсов',
    'battle.action.noAbilities': 'Нет доступных способностей',
    'battle.action.notYourTurn': 'Сейчас не ваш ход',
    'battle.action.waitingTurn': 'Дождитесь своей очереди',
    'battle.action.spell.level': 'Ур. {n}',
    'battle.action.spell.cantrip': 'Заговор',
    'battle.action.spell.cast': 'Заклинание использовано.',
    'battle.action.spell.castFailed': 'Не удалось использовать заклинание.',
    'battle.action.spell.castTitle': 'Использовать заклинание',
    'battle.action.spell.target': 'Цель: {name}',
    'battle.action.spell.selfTarget': 'Цель не выбрана — по себе / области.',
    'battle.action.spell.hint': 'Клик по токену на карте — выбрать цель. Слот тратит сервер; результат — в логе.',
    'battle.action.spell.upcast': 'Уровень слота (усиление)',
    'battle.action.spell.slotLevel': 'Слот {n}',
    'battle.action.spell.preview.damage': 'Урон: {dice}',
    'battle.action.spell.preview.save': 'спасбросок DC {dc}',
    'battle.action.spell.preview.half': 'половина при успехе',
    'battle.action.spell.preview.attack': 'бросок атаки заклинанием',
    'battle.action.spell.preview.heal': 'Лечение: {n}',
    'battle.action.spell.preview.manual': 'Часть эффекта GM разрешает вручную.',
    'battle.action.spell.damageRoll': 'Бросок урона',
    'battle.action.spell.auto': 'АВТО',
    'battle.action.spell.manual': 'Ручной',
    'battle.action.spell.manualDmg': 'Урон с кубиков',
    'battle.action.spell.manualHint': 'Киньте кубики сами и введите суммарный урон; сервер применит сейв и сопротивление стихии.',
    'battle.action.spell.autoHint': 'Приложение само бросит кубики и посчитает урон с учётом сопротивления.',
    'battle.action.spell.dealt': 'Нанесено урона: {n}',
    'battle.action.spell.direction': 'Направление',
    'battle.action.spell.directionDeg': 'Направление {n}°',
    'battle.action.spell.casting': 'Кастуем…',
    'battle.action.spell.blocked': 'Чтобы использовать: {reason}',
    'battle.action.spell.aoe': 'Область (AoE)',
    'battle.action.spell.aoePickCell': 'Кликните пустую клетку на карте — это центр/вершина области.',
    'battle.action.spell.aoeOrigin': 'Точка: {x}, {y}. Задетые цели:',
    'battle.action.spell.aoeNobody': 'В области никого — зона всё равно будет создана.',
    'battle.action.spell.preview.area': 'Область: {shape} {n} фт',
    'battle.action.spell.preview.difficult': 'труднопроходимая местность',
    'battle.action.spell.preview.obscure.LIGHT': 'слабо заслоняет',
    'battle.action.spell.preview.obscure.HEAVY': 'сильно заслоняет',
    'battle.action.spell.shape.SPHERE': 'сфера, радиус',
    'battle.action.spell.shape.CUBE': 'куб, ребро',
    'battle.action.spell.shape.CONE': 'конус, длина',
    'battle.action.spell.shape.CYLINDER': 'цилиндр, радиус',
    'battle.action.spell.shape.LINE': 'линия, длина',

    /* ── Action economy (действия / бонусные / легендарные) ─ */
    'battle.eco.title': 'Действия',
    'battle.eco.action': 'Действие',
    'battle.eco.bonus': 'Бонусное',
    'battle.eco.legendary': 'Легендарное',
    'battle.eco.gmTitle': 'Настройка действий',
    'battle.eco.apply': 'Применить',

    /* ── Spell slots ─────────────────────────────────────── */
    'battle.slots.title': 'Ячейки заклинаний',
    'battle.slots.level': '{n}-й круг',

    /* ── Attack flow (player + GM) ───────────────────────── */
    'battle.attack.title': 'Атака',
    'battle.attack.pickAttack': 'Выберите атаку',
    'battle.attack.pickTarget': 'Выберите цель',
    'battle.attack.noTargets': 'Нет доступных целей',
    'battle.attack.d20': 'Бросок d20',
    'battle.attack.rollMode': 'Режим броска',
    'battle.attack.mode.NORMAL': 'Обычный',
    'battle.attack.mode.ADVANTAGE': 'Преимущество',
    'battle.attack.mode.DISADVANTAGE': 'Помеха',
    'battle.attack.serverRoll': 'Бросает сервер',
    'battle.attack.rollDie': 'Бросить кубик',
    'battle.attack.confirm': 'Атаковать',
    'battle.attack.hint': 'Введите d20 (1–20) или бросьте кубик, сервер рассчитает попадание',
    'battle.attack.cover': 'Укрытие цели',
    'battle.attack.cover.NONE': 'Нет',
    'battle.attack.cover.HALF': 'Половина (+2)',
    'battle.attack.cover.THREE_QUARTERS': 'На ¾ (+5)',
    'battle.attack.cover.TOTAL': 'Полное',
    'battle.attack.cover.totalHint': 'Полное укрытие: цель нельзя атаковать напрямую',
    'battle.attack.rangeOverride': 'Игнорировать дистанцию (мастер)',
    'battle.attack.feet': 'фт.',
    'battle.attack.range.IN_REACH': 'В зоне досягаемости',
    'battle.attack.range.OUT_OF_REACH': 'Вне досягаемости',
    'battle.attack.range.IN_RANGE': 'В пределах дистанции',
    'battle.attack.range.LONG_RANGE': 'Большая дистанция — помеха',
    'battle.attack.range.RANGED_IN_MELEE': 'Стрельба в ближнем бою — помеха',
    'battle.attack.range.BEYOND_LONG_RANGE': 'За пределами дистанции',
    'battle.attack.outcome.HIT': 'Попадание',
    'battle.attack.outcome.MISS': 'Промах',
    'battle.attack.outcome.CRIT': 'Критическое попадание!',
    'battle.attack.dealt': 'Урон: {n} {type}',
    'battle.attack.rollLine': 'd20 {d20} · бонус {bonus} = {total} против КЗ {ac}',
    'battle.attack.targetDown': '{name} повержен(а)!',
    'battle.attack.targetHp': 'ОЗ цели: {cur}/{max}',

    /* ── GM active controls ─────────────────────────────── */
    'battle.gm.controls': 'Управление боем',
    'battle.gm.endTurn': 'Следующий ход',
    'battle.gm.endBattle': 'Завершить бой',
    'battle.gm.currentTurnOf': 'Ходит: {name}',
    'battle.gm.endBattleConfirm': 'Завершить бой? Отменить это действие нельзя.',
    'battle.gm.npcControl': 'Ход монстра',
    'battle.gm.npcAttackHint': 'Сейчас ходит монстр — выберите его атаку и цель',
    'battle.gm.hpTitle': 'Правка ОЗ',
    'battle.gm.hpPickTarget': 'Участник',
    'battle.gm.hpAmount': 'Значение',
    'battle.gm.hpDamage': 'Урон',
    'battle.gm.hpHeal': 'Лечение',

    /* ── Completed ──────────────────────────────────────── */
    'battle.completed.title': 'Бой завершён',
    'battle.completed.body': 'Сражение окончено.',
    'battle.completed.newBattle': 'Новый бой',

    /* ── Mobile section switch ──────────────────────────── */
    'battle.section.group': 'Отряд',
    'battle.section.bestiary': 'Бестиарий',
    'battle.section.tracker': 'Очередь',
    'battle.section.action': 'Действия',

    /* ── Toasts ─────────────────────────────────────────── */
    'battle.toast.created': 'Бой создан',
    'battle.toast.createFailed': 'Не удалось создать бой',
    'battle.toast.monsterAddFailed': 'Не удалось добавить монстра',
    'battle.toast.removeFailed': 'Не удалось убрать участника',
    'battle.toast.xpFailed': 'Не удалось изменить опыт',
    'battle.toast.started': 'Бой начался',
    'battle.toast.startFailed': 'Не удалось начать бой',
    'battle.toast.joined': 'Вы вступили в бой',
    'battle.toast.joinFailed': 'Не удалось вступить в бой',
    'battle.toast.turnEndFailed': 'Не удалось завершить ход',
    'battle.toast.ended': 'Бой завершён',
    'battle.toast.endFailed': 'Не удалось завершить бой',
    'battle.toast.dieRolled': 'Выпало: {n}',
    'battle.toast.attackFailed': 'Не удалось провести атаку',
    'battle.toast.hpFailed': 'Не удалось изменить ОЗ',
    'battle.toast.actionFailed': 'Не удалось изменить действия',
    'battle.standard.title': 'Стандартные действия',
    'battle.standard.DASH': 'Рывок',
    'battle.standard.DODGE': 'Уклонение',
    'battle.standard.DISENGAGE': 'Отход',
    'battle.standard.HELP': 'Помощь',
    'battle.standard.HIDE': 'Скрыться',
    'battle.standard.hint.DASH': 'Удваивает запас перемещения на этот ход',
    'battle.standard.hint.DODGE': 'Атаки по вам — с помехой, преимущество на спасбросках Ловкости',
    'battle.standard.hint.DISENGAGE': 'Перемещение не провоцирует атак этот ход',
    'battle.standard.pickAlly': 'Выберите союзника',
    'battle.standard.confirmHelp': 'Помочь',
    'battle.standard.stealthD20': 'd20 (Скрытность)',
    'battle.standard.stealthBonus': 'Бонус',
    'battle.standard.hideDc': 'DC (опц.)',
    'battle.standard.hideHint': 'Оставьте d20 пустым — сервер бросит сам; DC = пассивная Внимательность врагов',
    'battle.standard.confirmHide': 'Скрыться',
    'battle.standard.state.dashing': 'Рывок',
    'battle.standard.state.dodging': 'Уклонение',
    'battle.standard.state.disengaged': 'Отход',
    'battle.standard.state.hidden': 'Скрыт',
    'battle.standard.state.helpAdvantage': 'Помощь',
    'battle.standard.done.DASH': 'Рывок',
    'battle.standard.done.DODGE': 'Уклонение принято',
    'battle.standard.done.DISENGAGE': 'Отход выполнен',
    'battle.standard.done.HELP': 'Помощь оказана',
    'battle.standard.done.HIDE': 'Попытка скрыться',
    'battle.standard.failed': 'Не удалось выполнить действие',
    'battle.contest.GRAPPLE': 'Захват',
    'battle.contest.SHOVE': 'Толчок',
    'battle.contest.pickTarget': 'Выберите цель',
    'battle.contest.atkBonus': 'Ваш бонус',
    'battle.contest.defBonus': 'Бонус цели',
    'battle.contest.shove.PRONE': 'Сбить с ног',
    'battle.contest.shove.PUSH': 'Оттолкнуть',
    'battle.contest.hint': 'Оставьте бонусы — сервер бросит d20 за обоих; защитник выигрывает ничью',
    'battle.contest.confirm.GRAPPLE': 'Схватить',
    'battle.contest.confirm.SHOVE': 'Толкнуть',
    'battle.contest.won': 'Контест выигран: {total} против {target}',
    'battle.contest.lost': 'Контест проигран: {total} против {target}',
    'battle.contest.failed': 'Не удалось провести контест',
    'battle.reaction.title': 'Атака реакцией (OA)',
    'battle.reaction.hint': 'Выберите реагирующего — атака пройдёт вне его хода и потратит реакцию',
    'battle.reaction.pickReactor': 'Кто реагирует',
    'battle.reaction.noAttacks': 'У этого существа нет доступных атак',
    'battle.legendary.title': 'Легендарные возможности',
    'battle.legendary.action': 'Легендарное действие ({left}/{max})',
    'battle.legendary.resistance': 'Лег. сопротивление ({left}/{max})',
    'battle.legendary.resistanceUsed': 'Легендарное сопротивление: спасбросок автоуспех',
    'battle.legendary.failed': 'Не удалось применить',
    'battle.multiattack.remaining': 'Мультиатака: осталось атак — {n}',
    'battle.flying.badge': 'В полёте',
    'battle.flying.hover': 'Парит',
    'battle.flying.hint': 'Существо в воздухе',
    'battle.flying.hoverHint': 'Парит — не падает при скорости полёта 0',
    'battle.flying.takeoff': 'Поднять в воздух',
    'battle.flying.land': 'Приземлить',
    'battle.flying.failed': 'Не удалось изменить полёт',
    'battle.identity.hidden': 'Скрыт',
    'battle.identity.hiddenHint': 'Игроки видят «Неизвестное существо»',
    'battle.identity.hide': 'Скрыть личность от игроков',
    'battle.identity.reveal': 'Раскрыть личность',
    'battle.identity.failed': 'Не удалось изменить видимость личности',
  },
  en: {
    /* ── Tab + status ───────────────────────────────────── */
    'battle.tabLabel': 'Battle',
    'battle.status.ASSEMBLING': 'Assembling',
    'battle.status.ACTIVE': 'In progress',
    'battle.status.COMPLETED': 'Completed',

    /* ── Empty / start ──────────────────────────────────── */
    'battle.empty.player.title': 'No battle yet',
    'battle.empty.player.body': 'When the GM starts an encounter, it will appear here.',
    'battle.empty.gm.title': 'No battles',
    'battle.empty.gm.body': 'Assemble an enemy group and start a new battle.',
    'battle.start': 'Start battle',
    'battle.create.title': 'New battle',
    'battle.create.namePlaceholder': 'Battle name',
    'battle.create.confirm': 'Create',
    'battle.create.cancel': 'Cancel',
    'battle.create.defaultName': 'New encounter',

    /* ── Assembly (GM) ──────────────────────────────────── */
    'battle.assembly.title': 'Assembling battle',
    'battle.assembly.groupTitle': 'Enemy group',
    'battle.assembly.monstersTitle': 'Bestiary',
    'battle.assembly.empty': 'Add monsters from the list on the right.',
    'battle.assembly.add': 'Add',
    'battle.assembly.remove': 'Remove',
    'battle.assembly.confirmStart': 'Start battle',
    'battle.assembly.cancelBattle': 'Cancel assembly',
    'battle.assembly.searchPlaceholder': 'Search monster…',
    'battle.assembly.campaignMonsters': 'Campaign monsters',
    'battle.assembly.publicMonsters': 'Public bestiary',
    'battle.assembly.noMonsters': 'No monsters found.',
    'battle.assembly.needMonsters': 'Add at least one monster',

    /* ── Stats preview ──────────────────────────────────── */
    'battle.stats.avgDanger': 'Avg. danger',
    'battle.stats.totalXp': 'Total XP',
    'battle.stats.monsters': 'Monsters',
    'battle.stats.xpOverride': 'Battle XP',
    'battle.stats.xpAuto': 'Auto',
    'battle.stats.xpEditHint': 'Editable',
    'battle.stats.resetXp': 'Reset',
    'battle.stats.cr': 'CR',

    /* ── Tracker ────────────────────────────────────────── */
    'battle.tracker.title': 'Turn order',
    'battle.tracker.round': 'Round {n}',
    'battle.tracker.current': 'Current turn',
    'battle.tracker.init': 'Init',
    'battle.tracker.initHidden': "Monster's initiative is hidden by the GM",
    'battle.tracker.empty': 'No one has joined the battle yet.',
    'battle.tracker.you': 'You',
    'battle.tracker.conc': 'Conc.',
    'battle.tracker.concHint': 'Concentrating on a spell',
    'battle.tracker.monster': 'Monster',
    'battle.tracker.waiting': 'Waiting for players…',
    'battle.tracker.hp': 'HP',

    /* ── Join (player) ──────────────────────────────────── */
    'battle.join.title': 'Join battle',
    'battle.join.pickCharacter': 'Choose a character',
    'battle.join.noCharacters': 'You have no characters in this campaign.',
    'battle.join.initiative': 'd20 roll',
    'battle.join.rollDie': 'Roll die',
    'battle.join.manualHint': 'Enter a d20 value (1–20) or roll the die',
    'battle.join.serverRollHint': 'Leave empty and the server rolls the d20; or enter your own (1–20).',
    'battle.join.computed': 'Final initiative',
    'battle.join.formula': 'd20 {d20} · bonus {bonus}',
    'battle.join.confirm': 'Join',
    'battle.join.addAnother': 'Add another character',
    'battle.join.allJoined': 'All your characters are already in the battle.',

    /* ── Player action panel ────────────────────────────── */
    'battle.action.title': 'Your turn',
    'battle.action.resources': 'Resources',
    'battle.action.abilities': 'Abilities',
    'battle.action.ability.class': 'Class',
    'battle.action.ability.item': 'Items',
    'battle.action.ability.use': 'Use ability',
    'battle.action.ability.blocked': 'To use: {reason}',
    'battle.action.ability.manualOnly': 'manual',
    'battle.action.ability.previewLoading': 'Calculating ability plan…',
    'battle.action.ability.unavailable': 'Ability unavailable',
    'battle.action.ability.consumes': 'consumes',
    'battle.action.spells': 'Spells',
    'battle.action.slots': 'Spell slots',
    'battle.action.slot.level': 'Lvl {n}',
    'battle.action.economy': 'Actions',
    'battle.action.economy.action': 'Action',
    'battle.action.economy.bonus': 'Bonus',
    'battle.action.economy.reaction': 'Reaction',
    'battle.action.economy.used': 'Used',
    'battle.action.economy.free': 'Free',
    'battle.action.attacks': 'Attacks',
    'battle.action.items': 'Items',
    'battle.action.item.use': 'Use',
    'battle.action.item.used': 'Item used',
    'battle.action.item.failed': 'Could not use the item',
    'battle.action.item.hint': 'Consumables (potions, scrolls). The server checks the item is consumable.',
    'battle.action.effects': 'Active effects',
    'battle.action.endTurn': 'End turn',
    'battle.action.noResources': 'No resources',
    'battle.action.noAbilities': 'No abilities available',
    'battle.action.notYourTurn': 'Not your turn',
    'battle.action.waitingTurn': 'Wait for your turn',
    'battle.action.spell.level': 'Lvl {n}',
    'battle.action.spell.cantrip': 'Cantrip',
    'battle.action.spell.cast': 'Spell cast.',
    'battle.action.spell.castFailed': 'Failed to cast the spell.',
    'battle.action.spell.castTitle': 'Cast spell',
    'battle.action.spell.target': 'Target: {name}',
    'battle.action.spell.selfTarget': 'No target selected — self / area.',
    'battle.action.spell.hint': 'Click a token on the map to pick a target. The server spends the slot; result in the log.',
    'battle.action.spell.upcast': 'Slot level (upcast)',
    'battle.action.spell.slotLevel': 'Slot {n}',
    'battle.action.spell.preview.damage': 'Damage: {dice}',
    'battle.action.spell.preview.save': 'save DC {dc}',
    'battle.action.spell.preview.half': 'half on save',
    'battle.action.spell.preview.attack': 'spell attack roll',
    'battle.action.spell.preview.heal': 'Healing: {n}',
    'battle.action.spell.preview.manual': 'Part of the effect is adjudicated by the GM.',
    'battle.action.spell.damageRoll': 'Damage roll',
    'battle.action.spell.auto': 'AUTO',
    'battle.action.spell.manual': 'Manual',
    'battle.action.spell.manualDmg': 'Rolled total',
    'battle.action.spell.manualHint': 'Roll the dice yourself and enter the total; the server applies the save and elemental resistance.',
    'battle.action.spell.autoHint': 'The app rolls the dice and applies damage with resistance.',
    'battle.action.spell.dealt': 'Damage dealt: {n}',
    'battle.action.spell.direction': 'Direction',
    'battle.action.spell.directionDeg': 'Direction {n}°',
    'battle.action.spell.casting': 'Casting…',
    'battle.action.spell.blocked': 'To cast: {reason}',
    'battle.action.spell.aoe': 'Area (AoE)',
    'battle.action.spell.aoePickCell': 'Click an empty map cell — the area center/apex.',
    'battle.action.spell.aoeOrigin': 'Origin: {x}, {y}. Covered targets:',
    'battle.action.spell.aoeNobody': 'Nobody in the area — the zone is still created.',
    'battle.action.spell.preview.area': 'Area: {shape} {n} ft',
    'battle.action.spell.preview.difficult': 'difficult terrain',
    'battle.action.spell.preview.obscure.LIGHT': 'lightly obscures',
    'battle.action.spell.preview.obscure.HEAVY': 'heavily obscures',
    'battle.action.spell.shape.SPHERE': 'sphere, radius',
    'battle.action.spell.shape.CUBE': 'cube, edge',
    'battle.action.spell.shape.CONE': 'cone, length',
    'battle.action.spell.shape.CYLINDER': 'cylinder, radius',
    'battle.action.spell.shape.LINE': 'line, length',

    /* ── Action economy (action / bonus / legendary) ─────── */
    'battle.eco.title': 'Actions',
    'battle.eco.action': 'Action',
    'battle.eco.bonus': 'Bonus',
    'battle.eco.legendary': 'Legendary',
    'battle.eco.gmTitle': 'Adjust action economy',
    'battle.eco.apply': 'Apply',

    /* ── Spell slots ─────────────────────────────────────── */
    'battle.slots.title': 'Spell slots',
    'battle.slots.level': 'Level {n}',

    /* ── Attack flow (player + GM) ───────────────────────── */
    'battle.attack.title': 'Attack',
    'battle.attack.pickAttack': 'Choose an attack',
    'battle.attack.pickTarget': 'Choose a target',
    'battle.attack.noTargets': 'No available targets',
    'battle.attack.d20': 'd20 roll',
    'battle.attack.rollMode': 'Roll mode',
    'battle.attack.mode.NORMAL': 'Normal',
    'battle.attack.mode.ADVANTAGE': 'Advantage',
    'battle.attack.mode.DISADVANTAGE': 'Disadvantage',
    'battle.attack.serverRoll': 'Server rolls',
    'battle.attack.rollDie': 'Roll die',
    'battle.attack.confirm': 'Attack',
    'battle.attack.hint': 'Enter a d20 (1–20) or roll the die; the server resolves the hit',
    'battle.attack.cover': 'Target cover',
    'battle.attack.cover.NONE': 'None',
    'battle.attack.cover.HALF': 'Half (+2)',
    'battle.attack.cover.THREE_QUARTERS': 'Three-quarters (+5)',
    'battle.attack.cover.TOTAL': 'Total',
    'battle.attack.cover.totalHint': 'Total cover: the target cannot be attacked directly',
    'battle.attack.rangeOverride': 'Ignore range (GM)',
    'battle.attack.feet': 'ft',
    'battle.attack.range.IN_REACH': 'Within reach',
    'battle.attack.range.OUT_OF_REACH': 'Out of reach',
    'battle.attack.range.IN_RANGE': 'Within range',
    'battle.attack.range.LONG_RANGE': 'Long range — disadvantage',
    'battle.attack.range.RANGED_IN_MELEE': 'Ranged in melee — disadvantage',
    'battle.attack.range.BEYOND_LONG_RANGE': 'Beyond long range',
    'battle.attack.outcome.HIT': 'Hit',
    'battle.attack.outcome.MISS': 'Miss',
    'battle.attack.outcome.CRIT': 'Critical hit!',
    'battle.attack.dealt': 'Damage: {n} {type}',
    'battle.attack.rollLine': 'd20 {d20} · bonus {bonus} = {total} vs AC {ac}',
    'battle.attack.targetDown': '{name} is down!',
    'battle.attack.targetHp': 'Target HP: {cur}/{max}',

    /* ── GM active controls ─────────────────────────────── */
    'battle.gm.controls': 'Battle controls',
    'battle.gm.endTurn': 'Next turn',
    'battle.gm.endBattle': 'End battle',
    'battle.gm.currentTurnOf': 'Current: {name}',
    'battle.gm.endBattleConfirm': 'End the battle? This cannot be undone.',
    'battle.gm.npcControl': 'Monster turn',
    'battle.gm.npcAttackHint': 'A monster is acting — pick its attack and target',
    'battle.gm.hpTitle': 'Adjust HP',
    'battle.gm.hpPickTarget': 'Combatant',
    'battle.gm.hpAmount': 'Amount',
    'battle.gm.hpDamage': 'Damage',
    'battle.gm.hpHeal': 'Heal',

    /* ── Completed ──────────────────────────────────────── */
    'battle.completed.title': 'Battle completed',
    'battle.completed.body': 'The encounter is over.',
    'battle.completed.newBattle': 'New battle',

    /* ── Mobile section switch ──────────────────────────── */
    'battle.section.group': 'Group',
    'battle.section.bestiary': 'Bestiary',
    'battle.section.tracker': 'Turn order',
    'battle.section.action': 'Actions',

    /* ── Toasts ─────────────────────────────────────────── */
    'battle.toast.created': 'Battle created',
    'battle.toast.createFailed': 'Failed to create battle',
    'battle.toast.monsterAddFailed': 'Failed to add monster',
    'battle.toast.removeFailed': 'Failed to remove combatant',
    'battle.toast.xpFailed': 'Failed to update XP',
    'battle.toast.started': 'Battle started',
    'battle.toast.startFailed': 'Failed to start battle',
    'battle.toast.joined': 'You joined the battle',
    'battle.toast.joinFailed': 'Failed to join battle',
    'battle.toast.turnEndFailed': 'Failed to end turn',
    'battle.toast.ended': 'Battle ended',
    'battle.toast.endFailed': 'Failed to end battle',
    'battle.toast.dieRolled': 'Rolled: {n}',
    'battle.toast.attackFailed': 'Attack failed',
    'battle.toast.hpFailed': 'Failed to change HP',
    'battle.toast.actionFailed': 'Failed to update action economy',
    'battle.standard.title': 'Standard actions',
    'battle.standard.DASH': 'Dash',
    'battle.standard.DODGE': 'Dodge',
    'battle.standard.DISENGAGE': 'Disengage',
    'battle.standard.HELP': 'Help',
    'battle.standard.HIDE': 'Hide',
    'battle.standard.hint.DASH': 'Doubles your movement budget this turn',
    'battle.standard.hint.DODGE': 'Attacks against you have disadvantage; advantage on Dex saves',
    'battle.standard.hint.DISENGAGE': 'Your movement provokes no opportunity attacks this turn',
    'battle.standard.pickAlly': 'Choose an ally',
    'battle.standard.confirmHelp': 'Help',
    'battle.standard.stealthD20': 'd20 (Stealth)',
    'battle.standard.stealthBonus': 'Bonus',
    'battle.standard.hideDc': 'DC (opt.)',
    'battle.standard.hideHint': 'Leave d20 empty for the server to roll; DC = enemies’ passive Perception',
    'battle.standard.confirmHide': 'Hide',
    'battle.standard.state.dashing': 'Dash',
    'battle.standard.state.dodging': 'Dodge',
    'battle.standard.state.disengaged': 'Disengage',
    'battle.standard.state.hidden': 'Hidden',
    'battle.standard.state.helpAdvantage': 'Helped',
    'battle.standard.done.DASH': 'Dashing',
    'battle.standard.done.DODGE': 'Dodging',
    'battle.standard.done.DISENGAGE': 'Disengaged',
    'battle.standard.done.HELP': 'Ally helped',
    'battle.standard.done.HIDE': 'Hide attempted',
    'battle.standard.failed': 'Could not take the action',
    'battle.contest.GRAPPLE': 'Grapple',
    'battle.contest.SHOVE': 'Shove',
    'battle.contest.pickTarget': 'Choose a target',
    'battle.contest.atkBonus': 'Your bonus',
    'battle.contest.defBonus': 'Target bonus',
    'battle.contest.shove.PRONE': 'Knock prone',
    'battle.contest.shove.PUSH': 'Push away',
    'battle.contest.hint': 'Leave bonuses empty and the server rolls d20 for both; the defender wins ties',
    'battle.contest.confirm.GRAPPLE': 'Grapple',
    'battle.contest.confirm.SHOVE': 'Shove',
    'battle.contest.won': 'Contest won: {total} vs {target}',
    'battle.contest.lost': 'Contest lost: {total} vs {target}',
    'battle.contest.failed': 'Contest failed',
    'battle.reaction.title': 'Reaction attack (OA)',
    'battle.reaction.hint': 'Pick the reactor — the attack resolves out of turn and spends its reaction',
    'battle.reaction.pickReactor': 'Who reacts',
    'battle.reaction.noAttacks': 'This creature has no available attacks',
    'battle.legendary.title': 'Legendary options',
    'battle.legendary.action': 'Legendary action ({left}/{max})',
    'battle.legendary.resistance': 'Legendary Resistance ({left}/{max})',
    'battle.legendary.resistanceUsed': 'Legendary Resistance: save auto-succeeds',
    'battle.legendary.failed': 'Could not apply',
    'battle.multiattack.remaining': 'Multiattack: attacks left — {n}',
    'battle.flying.badge': 'Flying',
    'battle.flying.hover': 'Hovering',
    'battle.flying.hint': 'The creature is airborne',
    'battle.flying.hoverHint': 'Hovering — does not fall at 0 fly speed',
    'battle.flying.takeoff': 'Take off',
    'battle.flying.land': 'Land',
    'battle.flying.failed': 'Failed to change flight',
    'battle.identity.hidden': 'Hidden',
    'battle.identity.hiddenHint': 'Players see "Unknown creature"',
    'battle.identity.hide': 'Hide identity from players',
    'battle.identity.reveal': 'Reveal identity',
    'battle.identity.failed': 'Failed to change identity visibility',
  },
};
